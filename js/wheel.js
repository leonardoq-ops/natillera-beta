...
(function () {
    ...
    function build(svg) {
        ...
        radGrad('gCenter', [['0%','#FFFDF8'],['100%','#EDE0C4']]);
        ...
        MONTHS.forEach((_, i) => {
            ...
            const lower = mid > 90 && mid < 270;   // text would be upside-down
            let d;
            if (lower) {
                // reverse arc so text faces outward
        ...
        /* ── outer border ── */
        svg.appendChild(el('circle', {cx:CX,cy:CY,r:243,fill:'none',stroke:C.gold,'stroke-width':'2.5'}));
        ...
        MONTHS.forEach((name, i) => {
            ...
            const seg = el('path', {
                ...
                fill, stroke: C.gold, 'stroke-width':'0.5',
                class: `ws-${s}`
            });
            ...
            svg.appendChild(el('line', {x1:lx1,y1:ly1,x2:lx2,y2:ly2,stroke:C.gold,'stroke-width':'1.5'}));
        ...
        svg.appendChild(el('circle', {cx:CX,cy:CY,r:moonMid,fill:'none',stroke:C.navy,'stroke-width':moonW}));
        svg.appendChild(el('circle', {cx:CX,cy:CY,r:MOON_RING[0],fill:'none',stroke:C.gold,'stroke-width':'1.5'}));
        svg.appendChild(el('circle', {cx:CX,cy:CY,r:MOON_RING[1],fill:'none',stroke:C.gold,'stroke-width':'1.5'}));
        ...
        phases.forEach((lit, i) => {
            ...
            svg.appendChild(el('circle', {cx:mx,cy:my,r:mr,fill:'#040818'}));
            ...
            if (lit === 1) {
                ...
                if (lit === 0.5) {
                    ...
                    svg.appendChild(el('circle', {cx:offX,cy:my,r:mr*0.86,fill:'#040818','clip-path':`url(#${did})`}));
                } else {
                    // crescent (0.25)
                    svg.appendChild(el('circle', {cx:mx,cy:my,r:mr,fill:'#EDE278',opacity:'0.88','clip-path':`url(#${cid})`}));
                    const offX = waning[i] ? mx - mr*0.44 : mx + mr*0.44;
                    svg.appendChild(el('circle', {cx:offX,cy:my,r:mr*0.80,fill:'#040818'}));
                }
            }
        });
        ...
        svg.appendChild(el('circle', {cx:CX,cy:CY,r:GOLD_RING[0],fill:'none',stroke:C.gold_dark,'stroke-width':'1.5'}));
        svg.appendChild(el('circle', {cx:CX,cy:CY,r:GOLD_RING[1],fill:'none',stroke:C.gold_dark,'stroke-width':'1.5'}));
        for (let i = 0; i < 24; i++) {
        ...
        /* ── center circle ── */
        svg.appendChild(el('circle', {cx:CX,cy:CY,r:CENTER_R,fill:'url(#gCenter)',stroke:C.gold,'stroke-width':'2'}));
